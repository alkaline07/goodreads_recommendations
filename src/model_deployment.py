...
logger = get_logger("model-deployment")
...
class DeploymentLogger:
    ...
    def __init__(self, project_id: str):
        ...
        try:
            ...
            logger.warning("Cloud Logging not available", error=str(e))
            self.enabled = False
    ...
    def log_deployment_event(
        ...
        if self.enabled:
            try:
                ...
                logger.warning("Failed to log to Cloud Logging", error=str(e))

        logger.info("Deployment Event", event_type=event_type, model=model_name,
                    version=model_version, status=status)
...
class ModelDeployer:
    ...
    def __init__(self, config: DeploymentConfig):
        ...
        logger.info("ModelDeployer initialized", project=self.config.project_id,
                    region=self.config.region, endpoint=self.config.endpoint_display_name)
    ...
    def get_model_from_registry(
        ...
        """
        logger.info("Fetching model from registry", display_name=display_name)
        ...
        try:
            ...
            if not models:
                logger.error("No model found", display_name=display_name)
                return None
            ...
            if version:
                ...
                logger.info("Retrieved specific version", version=version)
            else:
                ...
                if target_version:
                    ...
                    logger.info("Retrieved version",
                                type="default" if default_version else "latest",
                                version=target_version.version_id)
                else:
                    model = parent_model
            ...
            logger.error("Error fetching model from registry", error=str(e),
    ...
    def get_or_create_endpoint(self) -> Optional[aiplatform.Endpoint]:
        ...
        """
        logger.info("Looking for endpoint", endpoint_name=self.config.endpoint_display_name)
        ...
        try:
            ...
            if endpoints:
                ...
                logger.info("Found existing endpoint", resource_name=endpoint.resource_name)
                return endpoint
            ...
            logger.info("Created endpoint", resource_name=endpoint.resource_name)
            return endpoint

        except Exception as e:
            logger.error("Error getting/creating endpoint", error=str(e))
            return None
    ...
    def deploy_model(
        ...
        try:
            ...
            deployed_model = model.deploy(
                ...
                traffic_split={deployed_model_name: self.config.traffic_split_percentage,
                               **current_traffic} if current_traffic else None,
            ...
            result["deployed_model_id"] = deployed_model.resource_name if hasattr(deployed_model,
                                                                                  'resource_name') else str(
                deployed_model)
...
def main():
    ...
    parser.add_argument("--endpoint-name", default="goodreads-recommendation-endpoint",
