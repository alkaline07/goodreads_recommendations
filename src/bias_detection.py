...
class BiasDetector:
    ...
    def __init__(self, project_id: Optional[str] = None):
        ...
        logger.info("BiasDetector initialized", project=self.project_id)
    ...
    def compute_slice_metrics(
        ...
        try:
            ...
            logger.error("Error computing metrics", slice_dimension=slice_dimension, error=str(e))
            return []
    ...
    def generate_recommendations(self, disparity_analysis: Dict) -> List[str]:
        ...
        if disparity_analysis["detailed_disparities"]:
            high_severity = [d for d in disparity_analysis["detailed_disparities"] if
                             d["severity"] == "high"]
            if high_severity:
            ...
            medium_severity = [d for d in disparity_analysis["detailed_disparities"] if
                               d["severity"] == "medium"]
            if medium_severity:
                recommendations.append(
    ...
    def detect_bias(
        ...
        """
        logger.info("Starting bias detection", model=model_name, dataset=dataset)

        all_metrics = []
        slice_definitions = self.get_slice_definitions()

        logger.info("Analyzing dimensions", count=len(slice_definitions))

        for dim_name, slice_expr, slice_label in slice_definitions:
            logger.info("Processing dimension", dimension=dim_name)
            ...
            logger.info("Dimension processed", dimension=dim_name, slices_count=len(metrics))

        logger.info("Computed slice metrics", total_slices=len(all_metrics))

        logger.info("Analyzing disparities")
    ...
    def _print_report_summary(self, report: BiasReport):
        """Log a summary of the bias report."""
        logger.info("Bias detection summary",
                    model=report.model_name,
                    dataset=report.dataset,
                    total_slices=len(report.slice_metrics),
                    disparity_count=len(report.disparity_analysis["detailed_disparities"]),
                    high_risk_slices=len(report.disparity_analysis["high_risk_slices"]),
                    recommendations_count=len(report.recommendations))

        if report.disparity_analysis["detailed_disparities"]:
            for disp in report.disparity_analysis["detailed_disparities"]:
                logger.warning("Performance disparity detected",
                               dimension=disp['dimension'],
                               severity=disp['severity'],
                               mae_range=round(disp['mae_range'], 4),
                               mae_cv=round(disp['mae_cv'], 4))

        if report.disparity_analysis["high_risk_slices"]:
            for risk in report.disparity_analysis["high_risk_slices"][:5]:
                logger.warning("High-risk slice identified",
                               slice=risk['slice'],
                               mae=round(risk['mae'], 4),
                               deviation_pct=round(risk['mae_deviation_pct'], 1))
    ...
    def save_report(self, report: BiasReport, output_path: str):
        ...
        logger.info("Bias report saved", path=output_path)
    ...
    def create_bias_metrics_table(
        ...
        try:
            ...
            if df.empty:
                logger.warning("No slice metrics to save, skipping BigQuery write",
                               model=report.model_name)
                return
            # Add metadata
            ...
            logger.info("Bias metrics table created", table=output_table, rows=len(df))

        except Exception as e:
            logger.error("Error creating bias metrics table", error=str(e), table=output_table)
